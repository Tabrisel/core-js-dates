...
function getNextFriday(date) {
  const d = new Date(date);
  const days = d.getDay();
  const dayToFriday = 5 - days;

  return dayToFriday <= 0
    ? new Date(d.setDate(d.getDate() + dayToFriday + 7))
    : new Date(d.setDate(d.getDate() + dayToFriday));
}
...
function formatDate(date) {
  const d = new Date(date);
  return d.toLocaleString('en', { timeZone: 'UTC' });
}
...
function getNextFridayThe13th(date) {
  ...
  for (let monthIndex = 0; monthIndex < 12; monthIndex += 1) {
    const day = new Date(year, monthIndex, 13);
    nextFriday = day.getDay() === 5 ? day : nextFriday;
    if (nextFriday) break;
  }
